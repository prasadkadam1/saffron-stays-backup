...
const Tents = () => {
  let { tents, setTents } = useContext(context)
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    let item = tents.find((tent) => tent.id === formData.id);
    if (!item) {
      axios.post(`http://localhost:5000/tents/`, formData).then(() => {
      ...
      axios.put(`http://localhost:5000/tents/${formData.id}`, formData).then(() => {
        axios.get("http://localhost:5000/tents").then((res) => {
          setTents(res.data);
          setFormData({
            campName: "",
  ...
  let handleDelete = (tent) => {
    axios.delete(`http://localhost:5000/tents/${tent.id}`).then(() => {
      axios.get('http://localhost:5000/tents').then((res) => {
        setTents(res.data)
      })
    })
  }
  const handleUpdate = (tent) => {
    const mergedData = {
      ...initialFormState,
      ...tent,
      prices: { ...initialFormState.prices, ...tent.prices },
      amenities: [...tent.amenities],
      address: { ...initialFormState.address, ...tent.address },
      about: {
        ...initialFormState.about,
        ...tent.about,
        images: [...tent.about?.images]
      },
      refundPolicy: tent.refundPolicy,
      roomsInACamp
        : tent.roomsInACamp
          ?.length ?
          tent.roomsInACamp
  ...
  return (
    <div className='flex'>
      <div className="w-[35vw] bg-gray-200 h-[100vh] p-4 overflow-auto">
        <h1 className="font-bold text-center">Add Tent</h1>
...
" className="p-2 border border-gray-400 rounded" onChange={handleChange} />
          ...
          <button type="submit" className="bg-amber-500 text-white p-2 rounded mt-4">Add Tent</button>
        </form>
      </div>
      <div className='flex flex-wrap justify-center gap-2 w-[75vw] h-[100vh] overflow-auto'>
        <div >
          {
            tents.length > 0 && tents.map((tent, i) => {
              return (
                <div key={i} className='w-[100%]  m-2 p-2'>
                  <h1 className='font-bold'>{tent.campName}</h1>
                  <div>
                    {tent.about.images.map((image, i) => {
                      return (
                  ...
                  <p>{tent.about.info}</p>
                  <button onClick={() => { handleUpdate(tent) }} className='px-3 bg-amber-500 m-2 text-white rounded py-1'>Update</button>
                  <button onClick={() => { handleDelete(tent) }} className='px-3 bg-amber-500 m-2 text-white rounded py-1'>Delete</button>
                </div>
              )
            })
          }
