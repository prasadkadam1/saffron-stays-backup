...
const Cottages = () => {
  let { cottages, setcottages } = useContext(context)
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    let item = cottages.find((cottage) => cottage.id === formData.id);
    if (!item) {
      axios.post(`http://localhost:5000/cottages/`, formData).then(() => {
      ...
      axios.put(`http://localhost:5000/cottages/${formData.id}`, formData).then(() => {
        axios.get("http://localhost:5000/cottages").then((res) => {
          setcottages(res.data);
          setFormData({
            campName: "",
  ...
  let handleDelete = (cottage) => {
    axios.delete(`http://localhost:5000/cottages/${cottage.id}`).then(() => {
      axios.get('http://localhost:5000/cottages').then((res) => {
        setcottages(res.data)
      })
    })
...
" className="p-2 border border-gray-400 rounded" onChange={handleChange} />
      ...
      <div className='flex flex-wrap justify-center gap-2 w-[75vw] h-[100vh] overflow-auto'>
        <div >
          {
            cottages.length > 0 && cottages.map((cottage, i) => {
              return (
