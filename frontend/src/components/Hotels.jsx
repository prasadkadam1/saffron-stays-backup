...
const Hotels = () => {
  let { hotels, sethotels } = useContext(context)
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    let item = hotels.find((cottage) => cottage.id === formData.id);
    if (!item) {
      axios.post(`http://localhost:5000/hotels/`, formData).then(() => {
      ...
      axios.put(`http://localhost:5000/hotels/${formData.id}`, formData).then(() => {
        axios.get("http://localhost:5000/hotels").then((res) => {
          sethotels(res.data);
          setFormData({
            campName: "",
  ...
  let handleDelete = (cottage) => {
    axios.delete(`http://localhost:5000/hotels/${cottage.id}`).then(() => {
      axios.get('http://localhost:5000/hotels').then((res) => {
        sethotels(res.data)
      })
    })
...
" className="p-2 border border-gray-400 rounded" onChange={handleChange} />
      ...
      <div className='flex flex-wrap justify-center gap-2 w-[75vw] h-[100vh] overflow-auto'>
        <div >
          {
            hotels.length > 0 && hotels.map((cottage, i) => {
              return (
