...
const Villas = () => {
  let { villas, setvillas } = useContext(context)
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    let item = villas.find((cottage) => cottage.id === formData.id);
    if (!item) {
      axios.post(`http://localhost:5000/villas/`, formData).then(() => {
      ...
      axios.put(`http://localhost:5000/villas/${formData.id}`, formData).then(() => {
        axios.get("http://localhost:5000/villas").then((res) => {
          setvillas(res.data);
          setFormData({
            campName: "",
  ...
  let handleDelete = (cottage) => {
    axios.delete(`http://localhost:5000/villas/${cottage.id}`).then(() => {
      axios.get('http://localhost:5000/villas').then((res) => {
        setvillas(res.data)
      })
    })
...
" className="p-2 border border-gray-400 rounded" onChange={handleChange} />
      ...
      <div className='flex flex-wrap justify-center gap-2 w-[75vw] h-[100vh] overflow-auto'>
        <div >
          {
            villas.length > 0 && villas.map((cottage, i) => {
              return (
