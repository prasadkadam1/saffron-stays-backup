...
const Treehouses = () => {
  let { treehouses, settreehouses } = useContext(context)
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    let item = treehouses.find((cottage) => cottage.id === formData.id);
    if (!item) {
      axios.post(`http://localhost:5000/treehouses/`, formData).then(() => {
      ...
      axios.put(`http://localhost:5000/treehouses/${formData.id}`, formData).then(() => {
        axios.get("http://localhost:5000/treehouses").then((res) => {
          settreehouses(res.data);
          setFormData({
            campName: "",
  ...
  let handleDelete = (cottage) => {
    axios.delete(`http://localhost:5000/treehouses/${cottage.id}`).then(() => {
      axios.get('http://localhost:5000/treehouses').then((res) => {
        settreehouses(res.data)
      })
    })
...
" className="p-2 border border-gray-400 rounded" onChange={handleChange} />
      ...
      <div className='flex flex-wrap justify-center gap-2 w-[75vw] h-[100vh] overflow-auto'>
        <div >
          {
            treehouses.length > 0 && treehouses.map((cottage, i) => {
              return (
