...
const Homestays = () => {
  let { homestays, sethomestays } = useContext(context)
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    let item = homestays.find((cottage) => cottage.id === formData.id);
    if (!item) {
      axios.post(`http://localhost:5000/homestays/`, formData).then(() => {
      ...
      axios.put(`http://localhost:5000/homestays/${formData.id}`, formData).then(() => {
        axios.get("http://localhost:5000/homestays").then((res) => {
          sethomestays(res.data);
          setFormData({
            campName: "",
  ...
  let handleDelete = (cottage) => {
    axios.delete(`http://localhost:5000/homestays/${cottage.id}`).then(() => {
      axios.get('http://localhost:5000/homestays').then((res) => {
        sethomestays(res.data)
      })
    })
...
" className="p-2 border border-gray-400 rounded" onChange={handleChange} />
      ...
      <div className='flex flex-wrap justify-center gap-2 w-[75vw] h-[100vh] overflow-auto'>
        <div >
          {
            homestays.length > 0 && homestays.map((cottage, i) => {
              return (
